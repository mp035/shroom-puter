...
#define P1PR A  // data port for pin 1 on keypad
#define P1PN 12 // data bit for pin 1 on keypad

#define P2PR B // data port for pin 2 on keypad
#define P2PN 0 // data bit for pin 2 on keypad

#define P3PR A // data port for pin 3 on keypad
#define P3PN 10 // data bit for pin 3 on keypad
...
uint8_t scan_for_keycode() // returns the ascii value of the key pressed, or '\0' if no key pressed.  The process aborts on the first key found, so combinations of keys can not be used.
{
	
    // start by checking column 0
    uint8_t keyvalue;
	keyvalue = 0;
    const uint8_t keycodes[3][4] = {{'1','4','7','*'},{'2','5','8','0'},{'3','6','9','#'}};
    //const uint8_t colvalues[3][4] = {{P3PR,P3PN},{P1PR,P1PN},{P5PR,P5PN}};
    unsigned int x;
