...
volatile int8_t target_duty_setting = 0;// negative values for cooling, positive for heating
...
void SysTick_Handler(void)
{
	...
	if (peltier_ramp_counter >=5000)// adjust duty every 5 seconds because that is how often the temperature value is updated.
	{
		...
		{
			if(target_duty_setting < peltier_duty_setting)
			{
				if (peltier_duty_setting > -100){
					peltier_duty_setting--;
				}
			}
			else if(target_duty_setting > peltier_duty_setting)
			{
				if (peltier_duty_setting < 100){
					peltier_duty_setting++;
				}
